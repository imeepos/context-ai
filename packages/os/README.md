...
- 错误观测：`system.errors` 提供 `byErrorCode`、`byReason`、`topReasons`、`byService`、`recent` 与 `trend`，支持 `service/servicePrefix/errorCode/windowMinutes/limit/order/offset/recentLimit/bucketMinutes` 查询约束；`system.errors.export` 支持 `json/csv` 导出、`gzip+base64` 压缩与签名，且支持 key 轮换与激活（`system.errors.keys.*`）
