...
- 错误观测：`system.errors` 提供 `byErrorCode`、`byReason`、`topReasons`、`byService`、`recent` 与 `trend`，支持 `service/servicePrefix/errorCode/windowMinutes/limit/order/offset/recentLimit/bucketMinutes` 查询约束；`system.errors.export` 支持 `json/csv` 导出
