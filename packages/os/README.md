...
- 回滚治理增强：`system.app.rollback.state.import` 对快照/报告结构做强校验；`system.app.rollback.stats` 支持 `soonToExpireWindowMs` 并返回 `soonToExpireSnapshots`、`oldestCreatedAt/newestCreatedAt`；`system.app.rollback.gc` 支持 `dryRun/limit` 且返回 `eligible`
