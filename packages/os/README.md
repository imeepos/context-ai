...
- 存储与调度：`store.set / store.get`、`scheduler.scheduleOnce / scheduler.scheduleInterval / scheduler.cancel / scheduler.list / scheduler.failures.clear`、retryable task + DLQ
- 通知能力：`notification.send / notification.list`（支持 dedupe window）
- 系统服务：`system.health`、`system.dependencies`、`system.metrics`、`system.audit`、`system.topology(含 boot order)`、`system.events`、`system.capabilities`、`system.capabilities.list`、`system.policy`、`system.policy.evaluate`、`system.net.circuit`、`system.scheduler.failures`、`system.snapshot`、`system.errors`、`notification.list`
